...
def install_csp(wmi_obj, host_ip):

    print('CSP installation has been started ' + host_ip + '\n' + 30*'*')
	
    try:
        connect_reg = winreg.ConnectRegistry('\\\\' + host_ip, winreg.HKEY_LOCAL_MACHINE)
		...
		if reg_value != '3.6.7777':
            os.system(r'M:\tools\PsExec.exe \\' + host_ip + r' -c C:\Temp\cspsetup.exe -silent -args /norestart')
            ...
            if wmi_obj.Win32_OperatingSystem()[0].osarchitecture == '64-bit':
                print(wmi_obj.Win32_OperatingSystem()[0].osarchitecture)
            ...
            print('Keys on ' + host_ip + ' was be deleted\n')
            time.sleep(10)
            winreg.CloseKey(open_key)
            winreg.CloseKey(connect_reg)
			
    except FileNotFoundError:
        print('Can not open path on - ' + host_ip + '. May be CryptoPro not installed!\n')
    return 0
